...
function formatSourceMeta(data){
  if(!data || !data.sourceSheetId) return "";

  const title = data.sourceSheetTitle || "Drop";
  const tags = Array.isArray(data.sourceTags) ? data.sourceTags : [];
  const tagText = tags.length
    ? " · " + tags.map(t => "#" + formatLoopTag(t)).join(" ")
    : "";

  return title + tagText;
}

function renderTaskItem(type, d){
  const data = d.data();
  const subtaskMeta = getSubtaskMeta(data);
  const sourceMeta = formatSourceMeta(data);
  const metaText = sourceMeta || subtaskMeta;
  ...
  if(data.sourceSheetId){
    const openDrop = document.createElement("button");
    openDrop.className = "delete-btn";
    openDrop.textContent = "Öppna Drop";

    openDrop.onclick = (e) => {
      e.stopPropagation();
      open360Loop(data.sourceSheetId);
    };

    controls.appendChild(openDrop);
  }
...
window.closeCurrentLayer = () => {
  ...
  const menuOpen =
    menuLayer ? !menuLayer.classList.contains("hidden") : false;
...
function openTaskDetail(type, id, data){
  ...
  const sourceBlock = document.getElementById("detail-source-block");
  const sourceTitle = document.getElementById("detail-source-title");
  const sourceTags = document.getElementById("detail-source-tags");
  const sourceButton = document.getElementById("detail-open-drop");

  if(detailTask.data.sourceSheetId){
    sourceBlock?.classList.remove("hidden");
    sourceButton?.classList.remove("hidden");

    if(sourceTitle){
      sourceTitle.textContent = detailTask.data.sourceSheetTitle || "Drop";
    }

    if(sourceTags){
      const tags = Array.isArray(detailTask.data.sourceTags) ? detailTask.data.sourceTags : [];
      sourceTags.textContent = tags.map(t => "#" + formatLoopTag(t)).join(" ");
    }
  } else {
...
window.openSourceDrop = () => {
  if(!detailTask || !detailTask.data.sourceSheetId) return;

  const sourceId = detailTask.data.sourceSheetId;

  closeTaskDetail();
  closeMenuView(true);
  open360Loop(sourceId);
};
...
let loopPendingOpenSheetId = null;
...
function extractLoopActions(text){
  return (text || "")
    .split(/\r?\n/)
    .map(line => line.trim())
    .filter(line => /^#action\s+/i.test(line))
    .map(line => line.replace(/^#action\s+/i, "").trim())
    .filter(Boolean);
}

function getLoopPreview(text){
  const clean = (text || "")
    .replace(/^#action\s+.*$/gmi, "")
    .replace(/#[a-zA-ZåäöÅÄÖ0-9_-]+/g,"")
    .trim();

  return clean ? clean.slice(0,120) : "Tom Drop";
}
...
function updateLoopHeader(){
  const filter = document.getElementById("loopActiveFilter");
  if(filter){
    filter.textContent = loopActiveTag === "all"
      ? "# Alla"
...
function renderLoopSheetList(){
  ...
  filtered.forEach(sheet => {
    const row = document.createElement("div");
    row.className = sheet.id === loopActiveSheetId
    ...
    const actions = extractLoopActions(sheet.text);
    title.textContent = (actions.length ? "⚡ " : "") + (sheet.title?.trim() || "Namnlös Drop");
...
async function syncLoopActions(sheet){
  if(!sheet || sheet.isTemp || !sheet.id) return;

  const oldActionsQuery = query(
    collection(db, "attGora"),
    where("sourceType", "==", "drop"),
    where("sourceSheetId", "==", sheet.id)
  );

  const oldActions = await getDocs(oldActionsQuery);

  await Promise.all(
    oldActions.docs.map(d => deleteDoc(doc(db, "attGora", d.id)))
  );

  const actions = extractLoopActions(sheet.text);

  await Promise.all(
    actions.map((actionText, index) => addDoc(collection(db, "attGora"), {
      text: actionText,
      ...
      sourceSheetTitle: sheet.title?.trim() || "Namnlös Drop",
      sourceTags: Array.isArray(sheet.tags) ? sheet.tags : []
    }))
  );
}
...
window.deleteLoopSheet = async () => {
  ...
  const ok = confirm("Radera denna Drop?");
  if(!ok) return;

  await syncLoopActions({ ...sheet, text:"" });
...
function openPendingLoopSheetIfPossible(){
  if(!loopPendingOpenSheetId) return;

  const sheet = loopSheets.find(s => s.id === loopPendingOpenSheetId);
  if(!sheet) return;
  ...
  loopPendingOpenSheetId = null;

  setTimeout(() => {
    document.getElementById("loopText")?.focus();
  },100);
}

window.open360Loop = (targetSheetId = null) => {
  const loop = document.getElementById("loop360");
  ...
  loopActiveTag = "all";
  loopActiveSheetId = null;
  loopPendingOpenSheetId = targetSheetId;

  if(window.innerWidth <= 900 && !targetSheetId){
  ...
  if(!loopUnsubscribe){
    ...
    loopUnsubscribe = onSnapshot(q, snap => {
      ...
      renderLoop();
      openPendingLoopSheetIfPossible();
    });
  } else {
    renderLoop();
    openPendingLoopSheetIfPossible();
